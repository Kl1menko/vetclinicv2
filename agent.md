...
## Тема (фіксована)
- Основний колір: `#7579EA` (фіолетово-синій, перевизначає `--teal-600`).
- Акцент: `#ff7a45` (coral, не змінювати).
- Шрифти: Inter Tight (заголовки), Inter (текст). Радіус карток: 14px.
- Конфіг у `THEME` константі в [App.jsx](src/App.jsx), застосовується через `useEffect` до `document.documentElement`.
...
- `AdminMessages`: видалення повідомлення підтверджується через `window.confirm`.
...
- Перед видаленням сутності у store — перевіряти активні залежності; в UI — показувати `window.confirm`.
