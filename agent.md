...
### 2026-04-30 — updates (SEO + admin permissions + clinical flow)

- SEO routing:
  - публічний роутинг переведено на унікальні URL (без одного спільного адреса),
  - додано slug-маршрути для послуг і статей (`/poslugy/{slug}`, `/statti/{slug}`),
  - sync із `history.pushState` + підтримка `popstate` (back/forward + refresh),
  - канонічна головна сторінка: `/home`; з `/` виконується redirect на `/home`.
- Public/mobile UI fixes:
  - сторінка `Ціни`: прибрано некоректний sticky sidebar на мобільних,
  - форми на мобільних: прибрано auto-zoom/scale-jump (iOS/Safari) через `font-size:16px` + `text-size-adjust`,
  - статті: оновлено `cover`-посилання, додано fallback при битому фото, уніфіковано розміри обкладинок.
- Admin UI/UX fixes:
  - sidebar меню: прибрано вертикальне розтягування пунктів; кнопка виходу притиснута вниз,
  - `Доступ обмежено` центрується в контентній області,
  - `AdminModal` має `max-height` + внутрішній scroll (не виходить за viewport),
  - у таблиці `Записи` скасовані записи візуально приглушені та неактивні,
  - у `Записи` сортування: від новіших дата+час до старіших (минулі нижче).
- Clinical flow (admin):
  - кнопка `На прийом` відкриває окремий режим прийому (діагноз/лікування/призначення/рецепт/документи),
  - `Редагувати` в записах обмежено до планувальних полів (послуга/дата/час),
  - у `store` медичний запис по `completed` створюється/оновлюється з повним протоколом прийому,
  - додано прикріплення документів (PDF/зображення) до прийому та відображення в медісторії тварини,
  - додано експорт рецепта (txt) з картки тварини.
- Roles & permissions:
  - доступ до адмін-розділів і дій привʼязано до `roles[].perms` (а не лише до hardcoded ролей),
  - role selector в admin працює з реальними ролями зі store; rename/delete ролей синхронізуються коректно,
  - введено action-level guards: створення/скасування записів, лікування, CRUD клієнтів/лікарів/послуг/статей, рольова матриця, експорт, налаштування.
- Admin pets/doctors polish:
  - у `AdminPets` уніфіковано кольори іконок тварин у списку і детальній картці (без “всі сині”),
  - у `AdminDoctors` поле послуг переведено на українські назви; збереження мапиться в service `id` для сумісності.
